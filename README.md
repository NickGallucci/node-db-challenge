...
RD uses a relational model, SQL is a language that would be used to query the RD.
- [X] Why do tables need a `primary key`?
The purpose of the primary key is to uniquely identify a row on the table in order to form a relationship with another table.
- [X] What is the name given to a table column that references the primary key on another table.
A foreign key
- [X] What do we need in order to have a _many to many_ relationship between two tables.
 It occurs when multiple records in a table are associated with multiple records in another table.
